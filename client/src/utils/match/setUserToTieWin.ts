...
export const setUserToTieWin = (tournamentId: any, selectedUser: string, usersChoseHome: any, usersChoseTie: any, usersChoseAway: any) => {
    // @ts-ignore
    let indexTieWin = usersChoseTie.findIndex((item: any) => item.nickname === selectedUser && item.tournamentId === tournamentId);
    if (indexTieWin === -1) {
        // @ts-ignore
        let indexAway = usersChoseAway.findIndex((item: any) => item.nickname === selectedUser && item.tournamentId === tournamentId);
        if (indexAway > -1) {
        ...
        // @ts-ignore
        usersChoseTie.push({nickname: selectedUser, tournamentId: tournamentId});
    }
    const answer = {
