...
export const setUserToAwayWin = (tournamentId: any, selectedUser: string, usersChoseHome: any, usersChoseTie: any, usersChoseAway: any) => {
    // @ts-ignore
    let indexAwayWin = usersChoseAway.findIndex((item: any) => item.name === selectedUser && item.tournamentId === tournamentId);
    if (indexAwayWin === -1) {
        // @ts-ignore
        let indexTie = usersChoseTie.findIndex((item: any) => item.name === selectedUser && item.tournamentId === tournamentId);
        if (indexTie > -1) {
        ...
        usersChoseAway.push({name: selectedUser, tournamentId: tournamentId});
    }
    const answer = {
